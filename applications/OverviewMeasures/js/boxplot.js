/** file changed by Hauke Bartsch, 2013 */
/**
...
function setBPValues(_lw, _q1, _m, _q3, _uw, _mo, _eo, _mi, _ma) {
  lowerWhisker = _lw;
  q1 = _q1;
  median = _m;
  q3 = _q3;
...
function roundVal(val){
  ...
  if (Math.round(result).toString().length > 6)
    result = Math.round(result/1000000 * 100)/100 + 'M';
  else if (Math.round(result).toString().length > 3)
    result = Math.round(result/1000 * 100)/100 + 'k';
...
lowerLabel.style.top = (mlowerWhisker-8) + "px";
...
medianLabel.style.top = (mmedian - 5) + "px";
...
upperLabel.style.top = (mupperWhisker - 3) + "px";
