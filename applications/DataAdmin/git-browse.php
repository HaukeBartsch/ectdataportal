...
$repo_directory = 'bla'; // does not exist, its ok this way
...
if (file_exists($repo_index)) {
  ...
  foreach ($r as $repo)
   ...
   syslog(LOG_INFO, 'current directory is:'.getcwd().' try to see/create git repo now');
   $d = getcwd();
   $repos = array(
		  $d."/../../data/.git"
		  );
   foreach ($repos as $repo) {
      if (!file_exists($repo)) {
         syslog(LOG_INFO, 'DataAdmin: create initial data repo');
         // create an initial repo and add files in projects
         $out = array();
         $command = "GIT_DIR=".$repo." git init";
         exec($command, &$out);

         $fls = getFilesFromDir( $d."../../data/" );
         foreach ($fls as $f) {
           $out = array();
           $command = "GIT_DIR=".$repo." git add ".$f;
           exec($command, &$out);
         }
         $out = array();
         $command = "GIT_DIR=".$repo." git commit -m\"initial checkin\"";
         exec($command, &$out);
      }
   }
}
